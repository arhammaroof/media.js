    ...
    function buttons(container) { 
        ...
        $("body").on("click", "#medea-ok-btn", function(e) { 
           e.stopPropagation();
            ...
            var targetFormFields = container.find(".medea-root");
            var processedObject = form2object(elements.form);
            processedObject = YAML.stringify(processedObject, 4)
            processedObject = processedObject.replace(/(?:\r\n|\r|\n)/g, '<br />');
            console.log(processedObject);
            var result = container.triggerHandler(events.MEDEA_SUBMITTED, [ processedObject ]);
        });
        //EXPORT BUTTON
        var exportbutton = button("primary")
        .html("export")
        //.addClass("btn-block")
        .attr("id", "medea-export-btn");

       $("body").on("click", "#medea-export-btn", function(e) { 
          e.stopPropagation();
          var targetFormFields = container.find(".medea-root");
          var processedObject = form2object(elements.form);
          processedObject = YAML.stringify(processedObject, 4)
          var FileSaver = require('file-saver');
          var blob = new Blob([processedObject], {type: "text/plain;charset=utf-8"});
          FileSaver.saveAs(blob, "export.yml");
    });

        buttonWrapperRight.append(exportbutton);
...
},{"file-saver":2,"yamljs":13}],2:[function(require,module,exports){
(function (global){
